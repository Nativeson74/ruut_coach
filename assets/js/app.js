...
(function(){
  ...
  const GOAL_META={hybrid:{label:"Hybrid Athlete",block:"Hybrid Capacity Block"},muscle:{label:"Build Muscle",block:"Muscle Foundation Block"},fatloss:{label:"Lose Fat",block:"Body Composition Block"},endurance:{label:"13-Mile Endurance",block:"Endurance Build Block"},general:{label:"General Fitness",block:"General Fitness Block"},maintain:{label:"Maintain Fitness",block:"Maintenance Block"}};
  function esc(v){return String(v ?? "").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));}
  function n(v){const x=Number(v);return Number.isFinite(x)?x:0}
  function save(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));}catch(e){}}
  function localISO(d=new Date()){const x=new Date(d);x.setMinutes(x.getMinutes()-x.getTimezoneOffset());return x.toISOString().slice(0,10)}
  function parseISODate(s){const d=new Date(String(s)+"T12:00:00");return Number.isFinite(d.getTime())?d:new Date()}
  function addDaysISO(s,days){const d=parseISODate(s);d.setDate(d.getDate()+days);return localISO(d)}
  function ensure(){state.coachV16=state.coachV16||{};state.coachV16.goal=state.coachV16.goal||"hybrid";state.dailyIntelligenceV174=state.dailyIntelligenceV174||{};state.dailyIntelligenceV174.version="17.4.2";state.dailyIntelligenceV174.lastActiveDate=state.dailyIntelligenceV174.lastActiveDate||localISO();state.dailyIntelligenceV174.missedWorkouts=Array.isArray(state.dailyIntelligenceV174.missedWorkouts)?state.dailyIntelligenceV174.missedWorkouts:[];state.dailyIntelligenceV174.trophies=Array.isArray(state.dailyIntelligenceV174.trophies)?state.dailyIntelligenceV174.trophies:[];state.dailyIntelligenceV174.goalWeight=state.dailyIntelligenceV174.goalWeight ?? null;state.weightLogV16=Array.isArray(state.weightLogV16)?state.weightLogV16:[];state.completed=Array.isArray(state.completed)?state.completed:[];state.liftSessions=Array.isArray(state.liftSessions)?state.liftSessions:[]}
  function gid(){ensure();return GOAL_META[state.coachV16.goal]?state.coachV16.goal:"hybrid"}
  function meta(){return GOAL_META[gid()]||GOAL_META.hybrid}
  function dayIndex(){return Math.max(1,Math.min(7,Number(state.dayIndex||1)))}
  function weekNumber(){return Math.max(1,Number(state.week||1))}
  function schedule(){return WEEKLY[gid()]||WEEKLY.hybrid}
  function todayTitle(){return schedule()[dayIndex()-1]||"Recovery"}
  function dateLabel(){try{return new Date().toLocaleDateString(undefined,{month:"numeric",day:"numeric",year:"2-digit"})}catch(e){return localISO()}}
  function typeFromTitle(t){if(/Upper|Lower|Full Body/.test(t))return"lift";if(/Recovery|Mobility|Optional/.test(t))return"rest";return"run"}
  function templateFromTitle(t){if(t==="Upper A")return"upperA";if(t==="Lower + Core")return"lowerCore";if(t==="Upper B")return"upperB";if(t==="Full Body")return"fullBody";return""}
  function activeWorkout(){const title=todayTitle(),type=typeFromTitle(title);if(type==="lift")return{type,title,templateId:templateFromTitle(title),structure:`${title} strength session`,purpose:"Build strength inside the current training block.",success:"Complete the planned lifts and save the session."};if(type==="rest")return{type,title,structure:"Recovery, mobility, stretching, walking, or full rest.",purpose:"Recover so training can continue.",success:"Finish feeling better."};return{type,title,structure:title,purpose:"Build conditioning in support of the selected goal.",success:"Finish controlled."}}
  function weights(){ensure();return (state.weightLogV16||[]).filter(x=>x&&x.date&&n(x.weight)>0).slice().sort((a,b)=>String(a.date).localeCompare(String(b.date)))}
  function avg(rows){return rows.length?rows.reduce((s,x)=>s+n(x.weight),0)/rows.length:null}
  function fw(v){return v===null||v===undefined||Number.isNaN(Number(v))?"—":Number(v).toFixed(1)}
  function body(){const rows=weights(),latest=rows[rows.length-1]||null,first=rows[0]||null;const delta=latest&&first?n(latest.weight)-n(first.weight):null;const best=rows.length?rows.reduce((b,x)=>n(x.weight)<n(b.weight)?x:b,rows[0]):null;const goal=state.dailyIntelligenceV174.goalWeight??null;const toGoal=latest&&goal?n(latest.weight)-n(goal):null;return{rows,latest,first,delta,best,goal,toGoal,avg7:avg(rows.slice(-7)),avg30:avg(rows.slice(-30))}}
  function weightStreak(){const rows=weights();if(!rows.length)return 0;const dates=new Set(rows.map(x=>x.date));let count=0,cursor=localISO();while(dates.has(cursor)){count++;cursor=addDaysISO(cursor,-1);if(count>365)break}return count}
  function workoutCount(){return (state.completed||[]).length}
  function strengthCount(){return (state.liftSessions||[]).length}
  function recoveryCount(){return n(state.recoverySessions||0)+(Array.isArray(state.recoverySessionLogV154)?state.recoverySessionLogV154.length:0)}
  function prCount(){const best={};(state.liftSessions||[]).forEach(s=>(s.exercises||[]).forEach(e=>(e.sets||[]).forEach(set=>{const name=String(e.name||"").toLowerCase();const score=n(set.weight)*n(set.reps);if(name&&(!best[name]||score>best[name]))best[name]=score})));return Object.keys(best).length}
  function trophyDefs(){return [["firstWorkout","🏁","First Workout","Complete your first workout.",workoutCount()>=1],["tenWorkouts","🔥","10 Workouts","Complete 10 workouts.",workoutCount()>=10],["fiftyWorkouts","🏅","50 Workouts","Complete 50 workouts.",workoutCount()>=50],["hundredWorkouts","🏆","100 Workouts","Complete 100 workouts.",workoutCount()>=100],["firstStrength","💪","First Strength Session","Save your first strength session.",strengthCount()>=1],["fiftyStrength","🦾","50 Strength Sessions","Save 50 strength sessions.",strengthCount()>=50],["firstPR","⭐","First PR","Log your first exercise PR.",prCount()>=1],["tenPRs","🌟","10 PRs","Build PR history across 10 exercises.",prCount()>=10],["firstRecovery","🧘","First Recovery","Complete your first recovery session.",recoveryCount()>=1],["twentyFiveRecovery","🛡️","25 Recovery Sessions","Complete 25 recovery sessions.",recoveryCount()>=25],["sevenWeighIns","⚖️","7-Day Weigh-In Streak","Log weight 7 consecutive days.",weightStreak()>=7],["thirtyWeighIns","📈","30-Day Weigh-In Streak","Log weight 30 consecutive days.",weightStreak()>=30]]}
  function updateTrophies(){ensure();const set=new Set(state.dailyIntelligenceV174.trophies||[]);trophyDefs().forEach(t=>{if(t[4])set.add(t[0])});state.dailyIntelligenceV174.trophies=[...set];save()}
  function missed(){ensure();return state.dailyIntelligenceV174.missedWorkouts||[]}
  function removeWeekOutsideGoals(){["today","dashboard"].forEach(id=>{const host=document.getElementById(id);if(!host)return;host.querySelectorAll(".coach-v161-week-card").forEach(el=>el.remove());host.querySelectorAll(".coach17-calendar").forEach(cal=>{const sec=cal.closest("section");if(sec)sec.remove();else cal.remove()})})}
  function shell(){document.body.classList.add("coach-v16");document.title="COACH";document.querySelectorAll(".v15-wordmark").forEach(e=>e.textContent="COACH");document.querySelectorAll(".v15-subbrand").forEach(e=>e.textContent="Train With Purpose.");document.querySelectorAll("header h1").forEach(e=>e.textContent="COACH")}
  function metricsPanel(){const m=body();return `<section id="coach1742BodyMetrics" class="coach1742-panel"><span class="coach1742-badge">Body Metrics</span><h3>Weight Trend</h3><p class="v15-muted">RENPHO can feed this through Apple Health → iOS Shortcut → COACH import/manual entry. Current panel uses COACH weight entries.</p><div class="coach1742-grid"><div class="coach1742-tile"><span>Current</span><strong>${fw(m.latest?.weight)} lb</strong></div><div class="coach1742-tile"><span>7-Day Avg</span><strong>${fw(m.avg7)} lb</strong></div><div class="coach1742-tile"><span>30-Day Avg</span><strong>${fw(m.avg30)} lb</strong></div><div class="coach1742-tile"><span>Trend</span><strong>${m.delta===null?"—":`${m.delta>0?"+":""}${m.delta.toFixed(1)} lb`}</strong></div><div class="coach1742-tile"><span>Goal</span><strong>${m.goal?fw(m.goal)+" lb":"—"}</strong></div><div class="coach1742-tile"><span>To Goal</span><strong>${m.toGoal===null?"—":`${m.toGoal>0?"+":""}${m.toGoal.toFixed(1)} lb`}</strong></div><div class="coach1742-tile"><span>Best Block Weight</span><strong>${m.best?fw(m.best.weight)+" lb":"—"}</strong></div><div class="coach1742-tile"><span>Weigh-In Streak</span><strong>${weightStreak()}</strong></div></div><div class="coach1742-actions"><button class="v1531-button-primary" onclick="coachV16OpenWeight()">Add Weight</button><button class="v1531-button-secondary" onclick="coach1742SetGoalWeight()">Set Goal</button></div></section>`}
  function trophiesPanel(limit=999){updateTrophies();const unlocked=new Set(state.dailyIntelligenceV174.trophies||[]),defs=trophyDefs();return `<section id="coach1742Trophies" class="coach1742-panel"><span class="coach1742-badge">Trophy Cabinet</span><h3>Awards</h3><p class="v15-muted">${unlocked.size} of ${defs.length} unlocked.</p><div class="coach1742-list">${defs.slice(0,limit).map(t=>{const on=unlocked.has(t[0]);return `<div class="coach1742-row ${on?"":"locked"}"><div class="coach1742-icon">${t[1]}</div><div><strong>${esc(t[2])}</strong><p>${esc(t[3])}</p></div><span class="coach1742-badge">${on?"Unlocked":"Locked"}</span></div>`}).join("")}</div></section>`}
  function rolloverPanel(){return `<section id="coach1742Rollover" class="coach1742-panel"><span class="coach1742-badge">Daily Rollover</span><h3>Midnight Advance</h3><p class="v15-muted">Active date: ${esc(state.dailyIntelligenceV174.lastActiveDate)}. After midnight, COACH advances the program and records unfinished workouts as missed.</p><div class="coach1742-grid"><div class="coach1742-tile"><span>Total Missed</span><strong>${missed().length}</strong></div><div class="coach1742-tile"><span>Current Program Day</span><strong>${weekNumber()}.${dayIndex()}</strong></div></div></section>`}
  function diagnosticPanel(){return `<section id="coach1742Diagnostic" class="coach1742-panel"><span class="coach1742-badge">Daily Intelligence</span><h3>Feature Status</h3><div class="coach1742-grid"><div class="coach1742-tile"><span>Version</span><strong>${esc(window.COACH_DAILY_INTELLIGENCE_VERSION||"17.4.2")}</strong></div><div class="coach1742-tile"><span>Trophies</span><strong>${(state.dailyIntelligenceV174.trophies||[]).length}</strong></div><div class="coach1742-tile"><span>Weight Entries</span><strong>${weights().length}</strong></div><div class="coach1742-tile"><span>Missed</span><strong>${missed().length}</strong></div></div><div class="coach1742-actions"><button class="v1531-button-primary" onclick="coach1742OpenTrophies()">Open Trophies</button><button class="v1531-button-secondary" onclick="coach1742OpenMetrics()">Open Metrics</button></div></section>`}
  function readinessPanel(){return `<section id="coach1742Readiness" class="coach163-status"><span class="coach163-pill">COACH Readiness</span><h3>Training Check</h3><p class="v15-muted">Use readiness reasoning to decide whether to proceed, hold, reduce, or recover.</p><div class="coach162-actions"><button class="v1531-button-primary" onclick="coach163OpenReadinessDetail()">View Reasoning</button><button class="v1531-button-secondary" onclick="coach162OpenProgressNote()">Add Note</button></div></section>`}
  function renderTodayHard(){ensure();shell();const host=document.getElementById("today");if(!host)return;const w=activeWorkout();host.innerHTML=`<section class="v15-today-hero coach-v16-hero"><div class="v15-hero-bg"></div><div class="v15-hero-shade"></div><div class="v15-hero-content"><div><div class="coach-v16-badge">COACH · ${esc(meta().label)}</div><div class="v15-meta-line">${dateLabel()} · Week ${weekNumber()} · Day ${dayIndex()}</div><h2 class="v15-hero-title">${esc(w.title)}</h2><div class="v15-hero-subtitle">${esc(w.structure)}</div></div><div class="v15-hero-bottom"><button class="v15-primary-action" onclick="coach17StartToday()">${w.type==="lift"?"Start Strength Log":"Start Guided Workout"}</button><button class="v15-round-action" onclick="window.openBriefingV110 ? window.openBriefingV110() : null">→</button></div></div></section>${readinessPanel()}<section class="coach1742-panel"><span class="coach1742-badge">Today</span><h3>${esc(w.success)}</h3><p class="v15-muted">${esc(w.purpose)}</p><div class="coach1742-actions"><button class="v1531-button-primary" onclick="showScreen('dashboard')">Stats</button><button class="v1531-button-secondary" onclick="showScreen('plan')">Goals</button></div></section>`}
  function weekGridGoals(){return `<section class="coach-v161-week-card"><div class="v15-kicker">This Week</div><h3>${esc(meta().label)} Schedule</h3><div class="coach17-calendar">${schedule().map((name,i)=>`<div class="coach17-day ${i+1===dayIndex()?"active":""}"><b>${DAYS[i]}</b><span>${esc(name)}</span></div>`).join("")}</div></section>`}
  function renderGoalsHard(){ensure();shell();const host=document.getElementById("plan");if(!host)return;host.innerHTML=`<section class="v15-screen-head"><div class="v15-kicker">Goals</div><h2>Training Calendar</h2><p class="v15-muted">The weekly schedule lives here only.</p></section><section class="coach-v16-goal-hero"><div class="coach-v16-badge">Active Goal</div><h2>${esc(meta().label)}</h2><p class="v15-muted">${esc(meta().block)}</p><div class="v15-metric-grid" style="margin-top:14px"><div><span>Week</span><strong>${weekNumber()}</strong></div><div><span>Day</span><strong>${dayIndex()}</strong></div></div><div class="coach-v16-actions"><button class="v1531-button-primary" onclick="coachV16OpenGoalPicker()">Change Goal</button><button class="v1531-button-secondary" onclick="openSetPosition ? openSetPosition() : null">Set Week/Day</button></div></section>${weekGridGoals()}`}
  function renderStatsHard(){ensure();shell();const host=document.getElementById("dashboard");if(!host)return;host.innerHTML=`<section class="v15-screen-head"><div class="v15-kicker">Stats</div><h2>Progress</h2><p class="v15-muted">Daily Intelligence, body metrics, awards, and rollover status.</p></section>${metricsPanel()}${trophiesPanel(6)}${rolloverPanel()}${diagnosticPanel()}`}
  window.coach1742OpenTrophies=function(){showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Awards</div><h2>Trophy Cabinet</h2><p>Streaks, workouts, PRs, recovery, and consistency.</p></div>${trophiesPanel()}<div class="coach1742-actions"><button class="v1531-button-primary" onclick="hideModal()">Done</button><button class="v1531-button-secondary" onclick="hideModal();showScreen('dashboard')">Stats</button></div>`)}
  window.coach1742OpenMetrics=function(){showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Body Metrics</div><h2>Weight Dashboard</h2><p>Low-friction trend tracking.</p></div>${metricsPanel()}<div class="coach1742-actions"><button class="v1531-button-primary" onclick="hideModal()">Done</button><button class="v1531-button-secondary" onclick="hideModal();showScreen('dashboard')">Stats</button></div>`)}
  window.coach1742SetGoalWeight=function(){const m=body();showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Body Metrics</div><h2>Goal Weight</h2><p>Set a target weight for this training block.</p></div><input id="coach1742GoalWeight" class="coach1742-tile" type="number" step="0.1" value="${esc(m.goal||"")}" placeholder="175"><div class="coach1742-actions"><button class="v1531-button-primary" onclick="coach1742SaveGoalWeight()">Save</button><button class="v1531-button-secondary" onclick="hideModal()">Cancel</button></div>`)}
  window.coach1742SaveGoalWeight=function(){ensure();const val=n(document.getElementById("coach1742GoalWeight")?.value);state.dailyIntelligenceV174.goalWeight=val||null;save();hideModal();try{renderStatsHard()}catch(e){}}
  window.coach1742Diagnostic=function(){return{version:window.COACH_DAILY_INTELLIGENCE_VERSION,dailyIntelligence:state.dailyIntelligenceV174,bodyMetricsPanel:!!document.querySelector("#coach1742BodyMetrics"),trophiesPanel:!!document.querySelector("#coach1742Trophies"),rolloverPanel:!!document.querySelector("#coach1742Rollover"),todayWeekCards:document.querySelectorAll("#today .coach-v161-week-card,#today .coach17-calendar").length,statsWeekCards:document.querySelectorAll("#dashboard .coach-v161-week-card,#dashboard .coach17-calendar").length,goalsWeekCards:document.querySelectorAll("#plan .coach-v161-week-card,#plan .coach17-calendar").length}}
  const oldShow=window.showScreen;window.showScreen=showScreen=function(id,btn){shell();document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active"));const screen=document.getElementById(id);if(screen)screen.classList.add("active");document.querySelectorAll("nav button").forEach(b=>b.classList.remove("active"));const navBtn=btn||document.querySelector(`nav button[data-target="${id}"]`);if(navBtn)navBtn.classList.add("active");if(id==="today")return renderTodayHard();if(id==="plan")return renderGoalsHard();if(id==="dashboard")return renderStatsHard();if(typeof oldShow==="function")return oldShow(id,btn)}
  window.renderToday=renderToday=renderTodayHard;window.renderPlan=renderPlan=renderGoalsHard;window.renderDashboard=renderDashboard=renderStatsHard;window.renderAll=renderAll=function(){shell();renderTodayHard();renderGoalsHard();renderStatsHard();try{if(typeof renderWorkout==="function")renderWorkout()}catch(e){}try{if(typeof renderRecover==="function")renderRecover()}catch(e){}try{if(typeof renderStrength==="function")renderStrength()}catch(e){}}
...
// ---------- COACH V17.7 BLOCK PROGRESSION + STATS CLEANUP ----------
(function(){
  const GOAL_BLOCKS={hybrid:{name:"Hybrid Capacity Block",length:12},muscle:{name:"Muscle Foundation Block",length:12},fatloss:{name:"Body Composition Block",length:12},endurance:{name:"Endurance Build Block",length:12},general:{name:"General Fitness Block",length:8},maintain:{name:"Maintenance Block",length:8}};
  function esc(v){return String(v ?? "").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));}
  function n(v){const x=Number(v);return Number.isFinite(x)?x:0}
  function todayISO(){const d=new Date();d.setMinutes(d.getMinutes()-d.getTimezoneOffset());return d.toISOString().slice(0,10)}
  function save(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}catch(e){}}
  function goalId(){return state.coachV16?.goal||"hybrid"}
  function blockMeta(){return GOAL_BLOCKS[goalId()]||GOAL_BLOCKS.hybrid}
  function ensure177(){
    state.trainingBlockV177=state.trainingBlockV177||{};
    const meta=blockMeta();
    state.trainingBlockV177.version="17.7";
    state.trainingBlockV177.id=state.trainingBlockV177.id||`block-${Date.now()}`;
    state.trainingBlockV177.goal=state.trainingBlockV177.goal||goalId();
    state.trainingBlockV177.name=state.trainingBlockV177.name||meta.name;
    state.trainingBlockV177.length=state.trainingBlockV177.length||meta.length;
    state.trainingBlockV177.started=state.trainingBlockV177.started||todayISO();
    state.trainingBlockV177.archives=Array.isArray(state.trainingBlockV177.archives)?state.trainingBlockV177.archives:[];
    state.trainingIntelligenceV176=state.trainingIntelligenceV176||{};
    state.trainingIntelligenceV176.ruutReadiness=Array.isArray(state.trainingIntelligenceV176.ruutReadiness)?state.trainingIntelligenceV176.ruutReadiness:[];
    state.completed=Array.isArray(state.completed)?state.completed:[];
    state.liftSessions=Array.isArray(state.liftSessions)?state.liftSessions:[];
  ...
  function weights(){return (state.weightLogV16||[]).filter(x=>x&&x.date&&n(x.weight)>0).slice().sort((a,b)=>String(a.date).localeCompare(String(b.date)))}
  function validSets(){const out=[];(state.liftSessions||[]).forEach(s=>(s.exercises||[]).forEach(ex=>(ex.sets||[]).forEach(set=>{if(ex.name&&n(set.weight)>0&&n(set.reps)>0)out.push({name:ex.name,weight:n(set.weight),reps:n(set.reps)})})));return out}
  function e1rm(set){return Math.round(n(set.weight)*(1+n(set.reps)/30))}
  function prCount(){const best={};validSets().forEach(set=>{const k=String(set.name).toLowerCase(),score=e1rm(set);if(!best[k]||score>best[k])best[k]=score});return Object.keys(best).length}
  function blockReport(){
    ensure177();const w=weights(),first=w[0]||null,last=w[w.length-1]||null,weightChange=first&&last?n(last.weight)-n(first.weight):null;
    const blockLength=n(state.trainingBlockV177.length||12),week=n(state.week||1),day=n(state.dayIndex||1),totalDays=blockLength*7,currentDay=Math.min(totalDays,((week-1)*7)+day),pct=Math.min(100,Math.round((currentDay/totalDays)*100));
    const completed=(state.completed||[]).length,attendance=totalDays?Math.round((completed/totalDays)*100):0;
    return{id:state.trainingBlockV177.id,name:state.trainingBlockV177.name,goal:state.trainingBlockV177.goal,started:state.trainingBlockV177.started,week,day,blockLength,totalDays,currentDay,pct,completed,attendance,strengthSessions:(state.liftSessions||[]).length,prs:prCount(),missed:(state.dailyIntelligenceV174.missedWorkouts||[]).length,awards:(state.dailyIntelligenceV174.trophies||[]).length,startWeight:first?n(first.weight):null,currentWeight:last?n(last.weight):null,weightChange};
  }
  function blockPanel(){const r=blockReport();return `<section id="coach177BlockProgression" class="coach177-panel"><span class="coach177-badge">Block Progression</span><h3>${esc(r.name)}</h3><p class="v15-muted">Week ${r.week} Day ${r.day} · ${r.pct}% through this ${r.blockLength}-week block.</p><div class="coach177-grid"><div class="coach177-tile"><span>Block Progress</span><strong>${r.pct}%</strong></div><div class="coach177-tile"><span>Attendance</span><strong>${r.attendance}%</strong></div><div class="coach177-tile"><span>Workouts</span><strong>${r.completed}</strong></div><div class="coach177-tile"><span>Strength Sessions</span><strong>${r.strengthSessions}</strong></div><div class="coach177-tile"><span>PRs</span><strong>${r.prs}</strong></div><div class="coach177-tile"><span>Weight Change</span><strong>${r.weightChange===null?"—":`${r.weightChange>0?"+":""}${r.weightChange.toFixed(1)} lb`}</strong></div></div><div class="coach177-actions"><button class="v1531-button-primary" onclick="coach177OpenBlockReport()">Open Block Report</button><button class="v1531-button-secondary" onclick="coach177OpenBlockHistory()">Block History</button></div></section>`}
  function statsCleanup(){const host=document.getElementById("dashboard");if(!host)return;["coach176Mount","coach1761Mount","coach1762Mount","coach177Mount"].forEach(id=>document.getElementById(id)?.remove());const seen={};host.querySelectorAll("section").forEach(sec=>{const text=(sec.innerText||"").trim();const key=text.includes("Readiness 2.0")?"readiness":text.includes("Strength Intelligence")?"strength":text.includes("Training Block Report")?"block":"";if(key){if(seen[key])sec.remove();else seen[key]=true;}})}
  function mountStats(){ensure177();const host=document.getElementById("dashboard");if(!host)return;statsCleanup();host.insertAdjacentHTML("afterbegin",`<div id="coach177Mount">${blockPanel()}</div>`)}
  function readinessDecision(entry){let score=70;if(entry.hrv!==null&&entry.hrv<25)score-=20;if(entry.rhr!==null&&entry.rhr>70)score-=12;if(entry.sleep!==null&&entry.sleep<6)score-=18;if(entry.exerciseMinutes!==null&&entry.exerciseMinutes>90)score-=8;if(entry.distance!==null&&entry.distance>8)score-=5;if(score>=75)return{color:"GREEN",score,line:"Train normally. Good to push if form is sharp."};if(score>=55)return{color:"YELLOW",score,line:"Maintain the plan. Do not chase max effort today."};return{color:"RED",score,line:"Recovery bias. Reduce load or swap for recovery."}}
  window.coach177OpenReadinessImport=function(){const last=(state.trainingIntelligenceV176?.ruutReadiness||[]).slice(-1)[0]||{};showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Manual Readiness Import</div><h2>Morning Metrics</h2><p>Enter numbers only. COACH does not automatically pull Apple Health or RENPHO data.</p></div><section class="coach177-panel"><div class="coach177-grid"><div class="coach177-field"><label>HRV</label><input id="coach177Hrv" class="coach177-input" type="number" step="0.1" value="${esc(last.hrv??"")}" placeholder="34"><p>Heart Rate Variability</p></div><div class="coach177-field"><label>Resting HR</label><input id="coach177Rhr" class="coach177-input" type="number" step="0.1" value="${esc(last.rhr??"")}" placeholder="61"><p>Resting heart rate</p></div><div class="coach177-field"><label>VO2 Max</label><input id="coach177Vo2" class="coach177-input" type="number" step="0.1" value="${esc(last.vo2??"")}" placeholder="42"><p>Cardio fitness estimate</p></div><div class="coach177-field"><label>Sleep Hours</label><input id="coach177Sleep" class="coach177-input" type="number" step="0.1" value="${esc(last.sleep??"")}" placeholder="7.2"><p>Previous night sleep</p></div><div class="coach177-field"><label>Exercise Minutes</label><input id="coach177Exercise" class="coach177-input" type="number" step="1" value="${esc(last.exerciseMinutes??"")}" placeholder="45"><p>Yesterday or latest total</p></div><div class="coach177-field"><label>Distance</label><input id="coach177Distance" class="coach177-input" type="number" step="0.1" value="${esc(last.distance??"")}" placeholder="4.1"><p>Walking/running distance</p></div></div><div class="coach177-actions"><button class="v1531-button-primary" onclick="coach177SaveReadinessImport()">Save Readiness</button><button class="v1531-button-secondary" onclick="hideModal()">Cancel</button></div></section>`)}
  window.coach177SaveReadinessImport=function(){ensure177();const val=id=>{const raw=document.getElementById(id)?.value;return raw===""||raw===undefined||raw===null?null:n(raw)};const entry={date:todayISO(),hrv:val("coach177Hrv"),rhr:val("coach177Rhr"),vo2:val("coach177Vo2"),sleep:val("coach177Sleep"),exerciseMinutes:val("coach177Exercise"),distance:val("coach177Distance")};entry.decision=readinessDecision(entry);state.trainingIntelligenceV176.ruutReadiness=state.trainingIntelligenceV176.ruutReadiness.filter(x=>x.date!==entry.date);state.trainingIntelligenceV176.ruutReadiness.push(entry);save();hideModal();try{renderDashboard()}catch(e){}alert(`Readiness saved: ${entry.decision.color} (${entry.decision.score}).`)}
  window.coach176OpenRuutImport=window.coach177OpenReadinessImport;
  window.coach177OpenBlockReport=function(){const r=blockReport();showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Training Block</div><h2>${esc(r.name)}</h2><p>Current block report.</p></div><section class="coach177-panel"><div class="coach177-grid"><div class="coach177-tile"><span>Started</span><strong>${esc(r.started)}</strong></div><div class="coach177-tile"><span>Progress</span><strong>${r.pct}%</strong></div><div class="coach177-tile"><span>Attendance</span><strong>${r.attendance}%</strong></div><div class="coach177-tile"><span>Workouts</span><strong>${r.completed}</strong></div><div class="coach177-tile"><span>Strength</span><strong>${r.strengthSessions}</strong></div><div class="coach177-tile"><span>PRs</span><strong>${r.prs}</strong></div><div class="coach177-tile"><span>Start Weight</span><strong>${r.startWeight===null?"—":r.startWeight.toFixed(1)+" lb"}</strong></div><div class="coach177-tile"><span>Current Weight</span><strong>${r.currentWeight===null?"—":r.currentWeight.toFixed(1)+" lb"}</strong></div><div class="coach177-tile"><span>Weight Change</span><strong>${r.weightChange===null?"—":`${r.weightChange>0?"+":""}${r.weightChange.toFixed(1)} lb`}</strong></div><div class="coach177-tile"><span>Missed</span><strong>${r.missed}</strong></div></div><div class="coach177-actions"><button class="v1531-button-primary" onclick="coach177CompleteBlock()">Complete Block</button><button class="v1531-button-secondary" onclick="hideModal()">Close</button></div></section>`)}
  window.coach177CompleteBlock=function(){ensure177();const report=blockReport();state.trainingBlockV177.archives.push({...report,completedAt:new Date().toISOString()});const meta=blockMeta();state.trainingBlockV177.id=`block-${Date.now()}`;state.trainingBlockV177.goal=goalId();state.trainingBlockV177.name=meta.name;state.trainingBlockV177.length=meta.length;state.trainingBlockV177.started=todayISO();state.week=1;state.dayIndex=1;save();hideModal();try{renderAll()}catch(e){}alert("Block archived. New block started at Week 1 Day 1.")}
  window.coach177OpenBlockHistory=function(){ensure177();const archives=(state.trainingBlockV177.archives||[]).slice().reverse();showModal(`<div class="v1531-modal-head"><div class="v15-kicker">Block History</div><h2>Archived Blocks</h2><p>Completed training blocks are stored here.</p></div><section class="coach177-panel"><div class="coach177-list">${archives.length?archives.map(b=>`<div class="coach177-row"><div class="coach177-icon">📦</div><div><strong>${esc(b.name)}</strong><p>${esc(b.started)} → ${esc(String(b.completedAt||"").slice(0,10))}</p><p>${b.completed} workouts · ${b.prs} PRs · ${b.attendance}% attendance</p></div><span class="coach177-badge">Block</span></div>`).join(""):`<p class="v15-muted">No archived blocks yet.</p>`}</div></section><div class="coach177-actions"><button class="v1531-button-primary" onclick="hideModal()">Done</button><button class="v1531-button-secondary" onclick="coach177OpenBlockReport()">Current Block</button></div>`)}
  const prevDashboard=window.renderDashboard;window.renderDashboard=renderDashboard=function(){if(typeof prevDashboard==="function")prevDashboard();mountStats();setTimeout(mountStats,100)}
  const prevShow=window.showScreen;window.showScreen=showScreen=function(id,btn){const result=typeof prevShow==="function"?prevShow(id,btn):undefined;if(id==="dashboard"||id==="stats"){setTimeout(mountStats,0);setTimeout(mountStats,150)}return result}
  ensure177();if(document.getElementById("dashboard")?.classList.contains("active"))mountStats();
...
// ---------- COACH v17.8.1 STATS DEDUPE ----------
(function(){
  function cleanupTrainingReportTitles(){
    const host=document.getElementById("dashboard");
    if(!host) return;
    ...
    [...host.querySelectorAll("section")].forEach(sec=>{
      ...
      if(txt.includes("training block report")){
        sec.querySelectorAll("h1,h2,h3,h4").forEach(h=>{
          const t=(h.innerText||"").trim().toLowerCase();
          if(
            t.includes("hybrid capacity block") ||
            t.includes("muscle foundation block") ||
            t.includes("body composition block") ||
            t.includes("endurance build block") ||
            t.includes("general fitness block") ||
            t.includes("maintenance block")
          ){
            h.innerText="Training Report";
          }
        });
      }
    });
  ...
  const prevDash181=window.renderDashboard;
  window.renderDashboard=renderDashboard=function(){
    if(typeof prevDash181==="function") prevDash181();
    setTimeout(cleanupTrainingReportTitles,50);
    setTimeout(cleanupTrainingReportTitles,250);
  };

  const prevShow181=window.showScreen;
  window.showScreen=showScreen=function(id,btn){
    const r=typeof prevShow181==="function" ? prevShow181(id,btn) : undefined;
    if(id==="dashboard"||id==="stats"){
      setTimeout(cleanupTrainingReportTitles,50);
      setTimeout(cleanupTrainingReportTitles,250);
    }
  ...
  window.COACH_STATS_DEDUPE_VERSION="17.8.1";
})();
